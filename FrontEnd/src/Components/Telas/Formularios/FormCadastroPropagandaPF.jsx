...
import { gravarPropagandaPF, alterarPropagandaPF } from "../../../Services/propagandaPFService";
import { FaSave, FaTimes } from "react-icons/fa";
import { buscarTodosClientePF } from "../../../Services/clientePFService";

export default function FormCadastroPropagandaPF({
    setExibirTabela,
    setAtualizarTela,
    modoEdicao,
    ...
    useEffect(() => {
        ...
        buscarTodosClientePF(token).then((resposta) => {
            if (resposta.status) {
    ...
    function manipularMudanca(evento) {
        ...
        setPropaganda((prev) => {
            ...
            // Verifica se ambos os campos estão preenchidos para calcular a duração
            if (novoEstado.data_emissao && novoEstado.data_encerramento) {
                ...
                novoEstado.duracao = dias >= 0 ? dias : ""; // Atualiza a duração somente se for válida
            }
    ...
    const manipularSubmissao = (evento) => {
        ...
        if (formulario.checkValidity()) {
            ...
            if (!modoEdicao) {
                gravarPropagandaPF(formData, token)
                    .then((resposta) => {
                ...
                alterarPropagandaPF(formData, token)
                    .then((resposta) => {
                        alert(resposta.mensagem);
        ...
        <Container>
            <Form noValidate validated={validado} onSubmit={manipularSubmissao} encType="multipart/form-data">
                ...
                {modoEdicao && (
                    <Row className="mb-5 d-flex justify-content-center">
                        <Col md="4">
                            <FloatingLabel controlId="cliente_cpf" label="CPF do Cliente" className="mb-3">
                                <Form.Control
                                    type="text"
                                    value={propaganda.cliente_cpf}
                                    readOnly
                                />
                            </FloatingLabel>
                        </Col>
