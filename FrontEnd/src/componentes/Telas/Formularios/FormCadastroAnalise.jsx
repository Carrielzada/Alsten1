...
const FormCadastroAnalise = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (form.checkValidity()) {
      console.log("Tipo de análise cadastrado:", analise);
      setValidado(false);
    } else {
      setValidado(true);
    }
  ...
  return (
    <Container className="p-3 border rounded shadow-sm mx-auto" style={{ maxWidth: '800px' }}>
      <Form noValidate validated={validado} onSubmit={handleSubmit}>
        <h5 className="mb-3 text-center fw-bold">Cadastro de Tipos de Análise</h5>
        <hr />
        <Row className="justify-content-center">
          <Col xs={12} md={6} className="mb-2">
            <FloatingLabel controlId="tipoAnalise" label="Tipo de Análise">
              <Form.Control
                size="sm"
                type="text"
